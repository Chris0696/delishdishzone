...
from accounts.views.users import check_role_vendor
...
def v_profile(request):
    ...
    if request.method == 'POST':
        ...
        if profile_form.is_valid() and vendor_form.is_valid():
            ...
            return redirect('accounts:vprofile')
        else:
...
def opening_hours(request):
    starded_hours = OpeningHour.objects.filter(vendor=get_vendor(request))
    form = OpeningHourForm()
    context = {
        'form': form,
        'opening_hours': starded_hours,
...
def orders_details(request, order_number):
    try:
    ...
    return render(request, 'vendor/orders_details.html', context)
